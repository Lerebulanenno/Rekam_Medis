<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($nama_err) && empty($tanggal_lahir_err) && empty($jenis_kelamin_err) && empty($alamat_err) && empty($telepon_err)) {
            // Prepare statement
            $sql_update = "UPDATE patients SET nama=?, tanggal_lahir=?, jenis_kelamin=?, alamat=?, telepon=? WHERE id=?";
            
            if ($stmt = $conn->prepare($sql_update)) {
                // Bind parameter ke statement
                $stmt->bind_param("sssssi", $nama, $tanggal_lahir, $jenis_kelamin, $alamat, $telepon, $id);
        ...
        $sql_select = "SELECT id, nama, tanggal_lahir, jenis_kelamin, alamat, telepon FROM patients WHERE id=?";
        ...
        if ($stmt = $conn->prepare($sql_select)) {
            ...
            if ($stmt->execute()) {
                ...
                if ($stmt->num_rows == 1) {
                    $stmt->bind_result($id, $nama, $tanggal_lahir, $jenis_kelamin, $alamat, $telepon);
                    $stmt->fetch();
                } else {
